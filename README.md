...
Regarding the switch, it was important to build the vlans to keep host-a and host-b in separate subnets. I installed the `openvswitch` packages(the installing commands were already provided) and created two differnt vlan with tag 2 and 3.

About host-2-c, it was requested to run on it a `docker image`. I found the command for insallation,downloading nginx-image and running docker called 'web-server' using nginx on port 80:80.
The commands were written down on file .sh 
...
The example below referred to host-b, but it's works also from host-a.
![alt text](https://github.com/marco-angheben/dncs-lab/blob/master/curl_screen.png)
